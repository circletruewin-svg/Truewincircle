...
function SmCard({card}){const red=card?.suit===“♥”||card?.suit===“♦”;return(<div className="w-9 h-12 rounded-lg border-2 border-gray-300 bg-white flex flex-col items-center justify-center text-xs font-bold shadow"><span className={red?“text-red-600”:“text-gray-900”}>{card?.rank}</span><span className={red?“text-red-600”:“text-gray-900”}>{card?.suit}</span></div>);}
const ROUND_SEC=12;
...
const[joker,setJoker]=useState(null);const[aCards,setACards]=useState([]);const[bCards,setBCards]=useState([]);
const[winner,setWinner]=useState(null);const[msg,setMsg]=useState(””);const[history,setHistory]=useState([]);const[timeLeft,setTimeLeft]=useState(ROUND_SEC);
...
const startRound=()=>{
clearInterval(timerRef.current);setPhase(“betting”);setBetSide(null);bsRef.current=null;baRef.current=null;hbRef.current=false;
...
setPhase(“dealing”);const j=rnd();setJoker(j);
const userBet=bsRef.current;const w=userBet?getBiasedWinner(userBet,[“andar”,“bahar”]):Math.random()>0.5?“andar”:“bahar”;
const acs=[],bcs=[];for(let i=0;i<5;i++){acs.push(rnd());bcs.push(rnd());}
let idx=0;const interval=setInterval(()=>{
if(idx<5){setACards(prev=>[…prev,acs[idx]]);setBCards(prev=>[…prev,bcs[idx]]);idx++;}
else{clearInterval(interval);setWinner(w);setPhase(“result”);finishRound(w,userBet);}
},400);
};
const finishRound=async(w,userBet)=>{
if(hbRef.current&&userBet){
const amt=baRef.current,won=w===userBet;const winAmt=won?parseFloat((amt*1.9).toFixed(2)):0;
...
return(<div className="min-h-screen bg-[#1a0a2e] text-white"><Navbar/>
<div className="max-w-lg mx-auto px-4 pb-8">
<h1 className="text-2xl font-black text-center text-pink-400 py-4">🎴 ANDAR BAHAR</h1>
<div className="flex gap-1.5 overflow-x-auto pb-2 mb-3">{history.map((h,i)=>(<span key={i} className={`px-2 py-1 rounded text-xs font-bold flex-shrink-0 ${h.winner==="andar"?"bg-purple-700":"bg-pink-700"}`}>{h.winner===“andar”?“A”:“B”}</span>))}</div>
...
<div className="flex justify-center mb-4">{joker?(<div className="text-center"><div className="text-xs text-yellow-400 font-bold mb-1">🃏 Joker</div><div className="w-16 h-24 rounded-2xl border-4 border-yellow-500 bg-white flex flex-col items-center justify-center"><span className={`text-2xl font-black ${joker.suit==="♥"||joker.suit==="♦"?"text-red-600":"text-gray-900"}`}>{joker.rank}</span><span className={`text-3xl ${joker.suit==="♥"||joker.suit==="♦"?"text-red-600":"text-gray-900"}`}>{joker.suit}</span></div></div>):(<div className="w-16 h-24 rounded-2xl border-4 border-yellow-500/30 bg-indigo-900/50 flex items-center justify-center"><span className="text-4xl">🂠</span></div>)}</div>
<div className="flex gap-3">
<div className="flex-1"><div className={`text-center font-bold text-sm mb-2 py-1 rounded-lg ${winner==="andar"?"bg-purple-600":"text-purple-400"}`}>ANDAR {winner===“andar”?“🏆”:””}</div><div className="flex flex-wrap gap-1 justify-center min-h-[3rem]">{aCards.map((c,i)=><SmCard key={i} card={c}/>)}</div></div>
<div className="flex-1"><div className={`text-center font-bold text-sm mb-2 py-1 rounded-lg ${winner==="bahar"?"bg-pink-600":"text-pink-400"}`}>BAHAR {winner===“bahar”?“🏆”:””}</div><div className="flex flex-wrap gap-1 justify-center min-h-[3rem]">{bCards.map((c,i)=><SmCard key={i} card={c}/>)}</div></div>
</div>
</div>
...
<button onClick={()=>placeBet(“andar”)} disabled={phase!==“betting”||hbRef.current} className={`${betSide==="andar"?"bg-purple-500 ring-2 ring-purple-300":"bg-purple-700 hover:bg-purple-600"} rounded-xl py-3 font-bold disabled:opacity-30`}>🟣 ANDAR {betSide===“andar”?“✅”:””}</button>
<button onClick={()=>placeBet(“bahar”)} disabled={phase!==“betting”||hbRef.current} className={`${betSide==="bahar"?"bg-pink-500 ring-2 ring-pink-300":"bg-pink-700 hover:bg-pink-600"} rounded-xl py-3 font-bold disabled:opacity-30`}>🩷 BAHAR {betSide===“bahar”?“✅”:””}</button>
</div>
</div>
</div>
