...
export default function ColorPrediction() {
      ...
      if (hasBetRef.current && userBet) {
        ...
        if (won) {
          ...
          await updateDoc(doc(db, "users", user.uid), {
            balance: balanceRef.current + winAmt,
          });
        } else {
  ...
  const placeBet = async (color) => {
    ...
    await updateDoc(doc(db, "users", user.uid), {
      balance: balanceRef.current - amt,
    });
  };
    ...
    <div className="min-h-screen bg-[#0d0d1a] text-white">
      ...
      <div className="max-w-lg mx-auto px-4 pb-8">
        ...
        <div className="text-center mb-4">
          {phase === "betting"
            ? <span className="text-lg font-bold">Bet closes in <span className="text-yellow-400 text-2xl">{timeLeft}s</span></span>
            : spinning
        ...
        <div className="flex justify-center mb-6">
          <div className={`w-28 h-28 rounded-full border-4 border-yellow-500 flex items-center justify-center text-5xl shadow-2xl
        ...
        <div className="bg-[#12152b] rounded-2xl p-4 border border-gray-800 mb-3">
          <div className="flex gap-2 mb-3">
            ...
            <div className="bg-gray-800 rounded-xl px-3 py-2.5 text-xs text-gray-400 whitespace-nowrap">₹{balance}</div>
          </div>
          ...
          {COLORS.map((c) => (
            <button key={c.id} onClick={() => placeBet(c.id)}
              ...
              className={`${c.bg} ${betColor === c.id ? `ring-4 ${c.ring}` : ""}
                ...
                disabled:opacity-40 disabled:cursor-not-allowed hover:opacity-90 active:scale-95 transition-all`}>
