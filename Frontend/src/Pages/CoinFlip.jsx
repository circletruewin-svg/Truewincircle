...
export default function CoinFlip() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, "users", user.uid), (s) => {
      if (s.exists()) {
        const d = s.data();
        const b = d.balance ?? d.walletBalance ?? 0;
  ...
  const flip = async () => {
    ...
    setTimeout(async () => {
      ...
      await addDoc(collection(db, "coinFlipHistory"), {
        userId: user.uid, betSide, result: outcome, betAmount: amt, winAmount: winAmt, won, createdAt: serverTimestamp(),
      });
    ...
    <div className="min-h-screen bg-[#0a1220] text-white">
      ...
      <div className="max-w-sm mx-auto px-4 pb-8">
        ...
        <div className="flex justify-center mb-6">
          <div className={`w-32 h-32 rounded-full border-4 border-yellow-500 flex items-center justify-center text-6xl shadow-2xl
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <button onClick={() => !locked && setBetSide("heads")} disabled={locked}
            className={`rounded-2xl py-5 flex flex-col items-center gap-1 font-bold transition-all
              ${betSide === "heads" ? "bg-yellow-600 ring-4 ring-yellow-400 scale-105" : "bg-yellow-800 hover:bg-yellow-700"} disabled:opacity-40`}>
            <span className="text-4xl">👑</span><span>HEADS</span><span className="text-xs opacity-80">1.9x</span>
          </button>
          <button onClick={() => !locked && setBetSide("tails")} disabled={locked}
            className={`rounded-2xl py-5 flex flex-col items-center gap-1 font-bold transition-all
              ${betSide === "tails" ? "bg-gray-500 ring-4 ring-gray-300 scale-105" : "bg-gray-700 hover:bg-gray-600"} disabled:opacity-40`}>
            <span className="text-4xl">🦅</span><span>TAILS</span><span className="text-xs opacity-80">1.9x</span>
          </button>
        </div>
        <div className="bg-[#12152b] rounded-2xl p-4 border border-gray-800">
          <div className="flex gap-2 mb-3">
            <input type="number" value={betAmount} onChange={(e) => setBetAmount(e.target.value)}
              placeholder="Bet amount (Min ₹10)" disabled={locked}
