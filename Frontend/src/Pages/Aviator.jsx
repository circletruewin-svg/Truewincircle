...
export default function Aviator() {
  ...
  // ── Listen to user balance (field: "balance") ──────────────
  useEffect(() => {
    if (!user) return;
  ...
  const startFlight = () => {
    ...
    animRef.current = setInterval(() => {
      ...
      setMultiplier(mult);
      setPlanePos({
        x: Math.min(5 + elapsed * 14, 82),
        y: Math.min(5 + elapsed * 9, 75),
      });
  ...
  const cashOut = async () => {
    ...
    try {
      await updateDoc(doc(db, "users", user.uid), {
        balance: balanceRef.current + win,
      });
      await addDoc(collection(db, "aviatorBets"), {
        userId: user.uid,
        betAmount: amt,
  ...
  return (
    <div className="min-h-screen bg-[#0b0d1a] text-white">
      ...
      <div className="max-w-lg mx-auto px-4 pb-8">
        ...
        {msg && (
          <div className="text-center text-sm font-semibold text-yellow-300 bg-yellow-900/20 rounded-xl py-2 px-3 mb-3">
            {msg}
          </div>
        )}
        ...
        <div className="bg-[#12152b] rounded-2xl p-4 border border-gray-800">
          <div className="flex gap-2 mb-3">
            ...
            <div className="bg-gray-800 rounded-xl px-3 py-2.5 text-xs text-gray-400 whitespace-nowrap">
              ₹{balance}
            </div>
          </div>
